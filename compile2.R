...
lapply(c('human','mouse','rat'),function(taxon){
    ...
    bc_path =  file.path(RAWDIR,'midway_backups',taxon,'differentials_single_pass.rds')
    ...
    if(load_from_backups && file.exists(bc_path)){
        ...
        all_differential_values <- differentials %>% pblapply(function(x){
            ...
            diff %<>% dplyr::filter(NCBIid!='' & !grepl('|',NCBIid,fixed = TRUE))
            p_value_cols = names(diff)[grepl('contrast.*?pvalue',names(diff))]
            logfc_cols = names(diff)[grepl('contrast.*?log2fc',names(diff))]
            stat_cols =names(diff)[grepl('contrast.*?tstat',names(diff))]
            coef_cols = names(diff)[grepl('contrast.*?coefficient',names(diff))]
            dup_genes = diff$NCBIid[duplicated(diff$NCBIid)]
            out = diff %>% dplyr::filter(!NCBIid %in% dup_genes) %>% dplyr::select(-Probe,-pvalue,-corrected_pvalue,-rank)
            ...
            lapply(seq_along(p_value_cols),function(i){
                diff %>% dplyr::filter(NCBIid %in% dup_genes) %>% dplyr::arrange(!!sym(p_value_cols[i])) %>%
                    dplyr::filter(!duplicated(NCBIid)) %>%
                    dplyr::select(NCBIid,gene_ensembl_id,GeneSymbol,GeneName,!!sym(coef_cols[i]),!!sym(logfc_cols[i]),!!sym(stat_cols[i]),!!sym(p_value_cols[i]))
            }) %>% {
                if(length(.)>1){
                    out = .[[1]]
